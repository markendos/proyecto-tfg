...
import es.upo.witzl.proyectotfg.users.repository.PasswordResetTokenRepository;
...
import es.upo.witzl.proyectotfg.users.repository.VerificationTokenRepository;
...
public class UserService implements IUserService {
    ...
    public void changeUserPassword(final User user, final String password) {
        ...
        if(hasPassResetToken(user)) {                //if the user has a password reset token
            Token resetToken = user.getResetToken();
            if(!isTokenExpired(resetToken)) {        //and token is not expired
                expireToken(resetToken);             //expire the current token
            }
        }
        userRepository.save(user);
    }
    ...
    @Override
    public boolean hasVerificationToken(User user) {
        return user.getVerificationTokentoken() != null;
    }

    @Override
    public boolean hasPassResetToken(User user) {
